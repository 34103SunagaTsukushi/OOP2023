    ...
    class Program {
        ...
        private static void Exercise03(List<int> numbers) {
            ...
            foreach(var number in orderNumbers) {
                ...
                if(number != orderNumbers.Last()) {//最後の空白をつけるか判断
                    Console.Write(" ");
                }
            }
        ...
        private static void Exercise04(List<int> numbers) {
            var numbersObj = numbers.Where(n => n >= 10 && n <= 50);
            foreach (var number in numbersObj) {
                Console.Write(number);
                if(number != numbersObj.Last()) {//最後の空白をつけるか判断
                    Console.Write(" ");
                }
            }
